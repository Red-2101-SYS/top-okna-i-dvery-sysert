...
export default function WindowsPage() {
  ...
  async function load() {
    setLoading(true);
    const res = await fetch("/api/products?type=WINDOW", { cache: "no-store" });
  ...
  return (
    <main style={{ padding: "24px 0" }}>
      <div style={ui.container}>
        ...
        >
          <div>
            <h1 style={{ fontSize: 28, fontWeight: 800, margin: 0 }}>Окна</h1>
            <p style={{ color: "#6b7280", marginTop: 8 }}>
              Рассчитаем стоимость • подберём профиль и стеклопакет
            </p>
          </div>

          <Link href="/calculator?tab=windows" className="btn btn-primary" style={ui.btnPrimary}>
            Рассчитать окно
          </Link>
        </div>
          ...
          >
            <label style={lbl}>
              ...
              <input
                ...
                placeholder="Например: двухкамерный"
                style={input}
              />
            </label>
          ...
          >
            {filtered.map((p) => (
              <div key={p.id} className="hoverCard" style={ui.card}>
                <Link href={`/windows/${p.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
                  ...
                  <div style={{ marginTop: 10, fontWeight: 700 }}>
                    {p.priceFrom ? `от ${p.priceFrom} ₽ /м²` : "Цена по запросу"}
                  </div>
                </Link>

                <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <Link href={`/windows/${p.slug}`} className="btn btn-ghost" style={ui.btnGhost}>
                    Подробнее
                  </Link>
                  <Link
                    href={`/calculator?tab=windows&product=${encodeURIComponent(p.slug)}`}
